#multi class, increased stopping min_delta, removed lots of nons from dataset...
...
#load dataset
df = pd.read_csv("../training-data/dataset/dataset_multiclass_features7.csv",dtype=np.float32)
...
callback = EarlyStopping(monitor='val_loss', patience=2, min_delta=0.0015)

#fit model - look up a good number of epochs and batches
model.fit(X_train, Y_train, epochs=60, batch_size=10,validation_data=(X_test, Y_test), callbacks=[callback])
...
model.summary()
model.save(filepath='../model/gestureClassifier/v17/model.h5',)
